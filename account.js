...
let configCache;
let sessionCache;
window.addEventListener('kubovistas-auth-changed',()=>{sessionCache=undefined;const route=(location.hash.slice(1).split('?')[0].split('/')[1]||'');if(accountRoutes.has(route)&&!['login','signup','forgot-password','reset-password','verify-email'].includes(route)){document.querySelector('main')?.replaceChildren();window.dispatchEvent(new Event('hashchange'));}void syncAccountButton().catch(()=>{});});
...
async function getConfig() {
  ...
  catch { configCache = { auth: false, database: false, emailVerification: false, google: false, payments: false, paymentProvider: 'Razorpay', currency: 'INR', advancePercent: 20 }; }
  return configCache;
}
...
export async function getAccountSession(force = false) {
  ...
  catch (error) { sessionCache = null; if(force)throw error; }
  return sessionCache;
}
...
function setupNotice(config) {
  ...
  return `<div class="setup-notice" role="status"><strong>Online accounts are temporarily unavailable</strong><p>Accounts are not available yet. Please return shortly.</p><a href="#/contact">Contact our travel team ↗</a></div>`;
}
...
async function resetPage(main, config) {
  const params = new URLSearchParams(location.hash.split('?')[1] || '');
  const token = params.get('oobCode') || params.get('token') || new URLSearchParams(location.search).get('oobCode') || '';
...
function verifyPage(main, config) {
  ...
  document.querySelector('#resend-verification')?.addEventListener('click', async event => {
    try { const resendButton=event.currentTarget; resendButton.disabled = true; await request('/api/auth/send-verification-email', { method: 'POST', body: JSON.stringify({ email, callbackURL: `${location.origin}/#/dashboard` }) }); document.querySelector('.form-status').textContent = 'A fresh verification link has been sent.'; }
    catch (error) { document.querySelector('.form-status').textContent = error.message; document.querySelector('#resend-verification')?.removeAttribute('disabled'); }
  });
}
...
function shell(main, active, session, title, intro, content) {
  if(session.renderVersion!==undefined && main.dataset.routeVersion!==session.renderVersion)return false;
...
export async function createConsultation(plan) {
  ...
  const payload={destination:plan.destination,days:plan.days,travelers:plan.travelers,style:plan.style,date:plan.date,budget:plan.budget,notes:plan.notes||'',itinerary:plan.itinerary||[]};
...
export const accountRoutes = new Set(['login','signup','forgot-password','reset-password','verify-email','welcome','dashboard','profile','settings','security','bookings','booking','payments','notifications','checkout','payment','confirmation','thank-you','cancellation-request','admin','saved','support']);
...
export async function renderAccountRoute(route, id, main, toast) {
  ...
  if(main.dataset.routeVersion!==version)return;
  const session=resolvedSession?{...resolvedSession,renderVersion:version}:null;
  ...
  if(['confirmation','thank-you','cancellation-request'].includes(route))return bookingFlowPage(route,id,main);
