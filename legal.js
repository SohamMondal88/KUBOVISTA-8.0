...
export const legalPages = {
  "privacy": {
    ...
    "sections": [{"title":"Optional Firebase Analytics and notifications","paragraphs":["The Google tag script loads on page load. Analytics collection is disabled by default until you enable it through Analytics preferences in the footer. Your choice is stored in this browser. When enabled, Google Analytics processes device and usage information; our public page-view events omit query strings, account identifiers and booking details. You can disable future collection in the footer. This does not erase data already collected or remove existing cookies.","If you enable booking push notifications in your account Updates page, we store the device token against your signed-in account to deliver generic trip alerts through Firebase Cloud Messaging. Permission is separate from Analytics. Full booking details stay behind sign-in. Disable push from Updates or revoke browser notification permission. Device registrations expire after 30 days without refresh; delivery records are cleaned up after seven days when the dispatcher runs. Signing out attempts to unregister the current device; if its token is no longer in memory, all account devices are unregistered."]},
      {"title":"Affiliate recommendations","paragraphs":["Where enabled, clearly labelled KuboVistas affiliate placements appear alongside journal content. We may receive commission on qualifying purchases or bookings. These placements are separate from a story author’s recommendations and do not imply the author has used or endorsed a product.","Selecting an affiliate link opens the provider’s website with its affiliate attribution parameters. The provider may use cookies or other identifiers to attribute a purchase under its own privacy notice and consent choices. This integration does not load affiliate tracking scripts, images or cookies before you select a link, and does not keep a first-party click log. Provider prices, availability, purchase terms and cancellation or return policies apply."]},
      {"title":"Travel Date companion community","paragraphs":["Travel Date stores your chosen display name, destination, trip dates, estimated budget, travel style, trip description, age attestation, join requests and their status. Plans are reviewed before publication and visible to signed-in, email-verified members. Your account email and private profile fields are not included in companion listings. Age is self-declared; we do not claim identity or background verification.","Join introductions are visible to the relevant organiser and requester. Blocking hides the other member’s plans and requests from your account and withdraws active requests between you. Reports and moderation records are available to authorised staff for review. An accepted request does not reveal contact details automatically. Authorised staff can access the account emails for accepted introductions to contact each member separately; contact details are shared between travellers only with their agreement. Contact the team for a separately agreed introduction.","Closing a plan stops public discovery but preserves it in your account history. Companion data is removed when the associated account is deleted, including linked requests; moderation records attached to those plans are also removed. Contact support for assistance. Never include identity documents, home addresses, payment information or other sensitive data in posts or introductions."]},
      {
        ...
        "paragraphs": [
          "The site includes the Google AdSense publisher script, which connects your browser to Google when the page loads. Ad display depends on publisher approval and configuration. Google and its partners may process device, connection and advertising information. Use the advertising consent message where applicable; Analytics preferences do not control AdSense.",
    ...
    "sections": [
      {
        ...
        "paragraphs": [
          "KuboVistas uses Firebase Authentication. Depending on your Keep me signed in choice, Firebase stores authentication state in browser local or session storage and refreshes short-lived identity tokens. Protected requests send identity tokens to our server for verification; old authentication cookies no longer grant access. The Google AdSense script loads with the site. Google and its advertising partners may process IP addresses, browser information, cookies and similar identifiers for advertising, measurement and fraud prevention. Checkout providers may use their own essential security technology."
        ]
      },
      {
