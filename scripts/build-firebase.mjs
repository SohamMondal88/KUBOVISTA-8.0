...
export async function buildFirebase(outdir = new URL('../dist/', import.meta.url)) {
  ...
  await build({ absWorkingDir: root, entryPoints: { 'firebase-client': 'client/firebase.js', 'firebase-auth-client': 'client/firebase-auth.js', 'firebase-firestore-client': 'client/firestore.js' }, outdir: destination, bundle: true, splitting: true, format: 'esm', platform: 'browser', target: ['es2020'], minify: true, chunkNames: 'firebase-chunk-[hash]' });
