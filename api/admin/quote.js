...
export default async function handler(req, res) {
  ...
  try {
    ...
    let cancellationPolicy;try{cancellationPolicy=validateCancellationPolicy(body.cancellationPolicy);}catch(error){return json(res,400,{error:error.message});}
    const checkin=new Date(body.checkinAt);if(!Number.isFinite(checkin.getTime())||checkin<=new Date())return json(res,400,{error:'Provide a future check-in date and time.'});
