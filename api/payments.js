...
export default async function handler(req, res) {
  ...
  if (!session) return;
  try {
    const result = await query(`SELECT p.id,p.booking_id,p.razorpay_order_id,p.razorpay_payment_id,p.amount_paise,p.currency,p.status,p.signature_verified,p.created_at,p.updated_at,b.destination_name
